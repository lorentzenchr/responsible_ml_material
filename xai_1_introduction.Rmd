...
Throughout the XAI notebooks, we consider a typical modeling situation: a distributional property $T$ of a real-valued response variable $Y$ should be approximated by a model $m: \boldsymbol x \in \mathbb R^p \mapsto \mathbb R$ of a $p$-dimensional feature vector $\boldsymbol X = (X^{(1)}, \dots, X^{(p)})^T$ with value $\boldsymbol x = (x^{(1)}, \dots, x^{(p)})^T \in \mathbb R^p$, i.e.,
$$
...
library(tidyverse)
library(gridExtra)
library(OpenML)   # devtools::install_github("openml/openml-r")
library(farff)
