...
- The additive LightGBM model performs much better than the (classic) GAM model, probably a result of its non-smooth effects.
...
- The model performs only slightly worse than without monotonicity constraints, even without re-tuning of the parameters.
...
For all models, we will use the features specified in `xvars`.
