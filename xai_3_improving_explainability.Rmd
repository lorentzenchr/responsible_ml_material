...
How do monotonic constraints work? To enforce monotonicity for the $j$th feature in a decision tree, the rule is as follows: A candidate split that violates the condition on its child nodes receives an infinite penalty (or *negative* split gain) and is therefore discarded (Scheme 3a). Node values of child nodes are capped at the average node values of the constraining split to avoid violation of monotonicity, see Scheme 3b.
...
![Scheme 3a: Monotonic constraints](figs/monotonic.jpg)
Scheme 3b explains how node values of subsequent splits are capped to enforce monotonicity (all credits to Samuel Flückiger, thank you!):

![Scheme 3b: Monotonic constraints](figs/monotonic_samuel_flueckiger.png)
