...
A *tree stump* is a decision tree with a single split. Its prediction function has the form $v_1 + (v_2 - v_1)\mathbb I(x^{(j)} \le s)$, where $\mathbb I$ is the indicator function, $s$ the split position of the selected variable $X^{(j)}$, and $v_1, v_2$ are the values of the left and right leaf. A boosted trees model with tree stumps as base learners (boosted tree stumps) is thus an additive model
$$
...
Interaction constraints can be used to create partly additive boosted trees models. For instance, to have a model additive in feature $X^{(j)}$, we need to set $F_m = \{X^{(j)}\}$, and make sure that $X^{(j)} \notin F_k, \ k \ne m$.
...
2. Forward step: Use $\hat \beta$ to calculate predictions $\hat y_1, \dots, \hat y_K$ of all $K$ observations in a *batch*. A batch is a small subset of the training data.
3. Backpropagation step: Modify $\hat \beta$ to improve the total loss $S(\hat\beta, D) = \sum_{i = 1}^K L(\hat y_i, y_i)$ on the batch for the given loss function $L$ (e.g., the squared error). This is done by gradient descent: Calculate the vector of partial derivatives
$$
  \nabla \hat \beta = \frac{\partial S(\beta, D)}{\partial \beta}\large\mid_{\beta = \hat \beta}
$$
at the current estimate $\hat \beta$. Use it to update
$$
  \hat \beta \leftarrow \hat \beta  - \lambda \nabla \hat \beta,
$$
where $\lambda > 0$ is the learning rate chosen to be small enough to improve $S$. In neural nets, parameters are organized in multiple layers, which makes it difficult to calculate $\nabla \hat\beta$. This is where backpropagation enters the game: It calculates the partial derivatives layer per layer using the chain rule, starting from the output layer.
4. Repeat Steps 2 and 3 until each observation appeared in a batch. This is called an *epoch*.
5. Repeat Step 4 for multiple epochs, usually until $\hat \beta$ stabilizes or validation performance stops improving.
