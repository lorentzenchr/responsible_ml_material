...
for (prop in dims$prop) {
  ...
  for (splittype in dims$splittype) {
    if (splittype == "random") {
      ...
      ix <- df_$group %in% sample(df_$group, n * 0.8)
    }
    df_train <- df_[ix, ]
    df_valid <- df_[!ix, ]
...
An alternative to the partial dependence plot, the *accumulated local effects* (ALE) plot [@apley2016], attempts to overcome the problem with the sometimes unnatural Ceteris Paribus clause. However, ALE plots are more difficult to calculate and to interpret. We will not consider them here.
...
- The contribution (total expected sum of payoffs) of a subset $\mathcal L \subseteq \mathcal M$ of the players is measured by a function $v: \mathcal L \mapsto \mathbb R$.
...
$$
Thus, a player's contribution is equal to the weighted average of his or her contribution to each possible coalition $\mathcal L$ of other players. The weights ensure that each coalition size $0 \le |\mathcal L| \le p-1$ receives equal weight, which follows from rewriting
$$
  \frac{|\mathcal L|!(p - |\mathcal L| - 1)!}{p!} = \frac{1}{p}\frac{1}{{p-1 \choose |\mathcal L|}}.
$$
$\phi_j$ is thus the average of the $p$ average contributions of player $j$ to coalitions of fixed size $0 \le |\mathcal L| \le p-1$. In Exercise 6iv, we will show that the Shapley value of player $j$ can also be interpreted as her (unweighted) average contribution over all permutations of the $p$ players.
...
- $p = 1$, i.e., there is only a single feature $x$. Here, the contribution of $x$ is simply $\phi_1 = m(x) - \phi_o$.
...
2. Heuristics: Several heuristics are available that try to identify the feature that explains most of the vertical scatter in the dependency plot conditional on the value on the horizontal axis. The selected feature is then visualized on the color axis.
...
- The SHAP dependence plots show quite realistic effects. For most features they give a similar impression as the corresponding partial dependency plots.
...
6.  SHAP (use only the XGBoost model here)
    ...
    iv. Why can we say: "The Shapley value of player $j$ equals her average contribution over all permutations of the $p$ players?"
